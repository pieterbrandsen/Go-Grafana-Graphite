...
export class Users {
  ...
  static async CreateUser (user: User): Promise<User> {
    const query = `
        INSERT INTO users (username, email, github_user_id)
        VALUES ('${user.username}', '${user.email}', '${user.github_user_id}')
        RETURNING *;
        `
    const res = await Query<User>(query)
    return res[0]
  ...
  static async UpdateUser (user: User): Promise<User> {
    const query = `
        UPDATE users SET username = '${user.username}', email = '${user.email}', github_user_id = '${user.github_user_id}' WHERE user_id = ${user.user_id};
        `
    const res = await Query<User>(query)
    return res[0]
  }
